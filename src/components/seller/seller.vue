...
<script>
  ...
  export default {
    ...
    methods: {
      updateSeller () {
        this.$nextTick(() => {
          ...
          const liWidth = 120
          const space = 6
          const imgSize = this.seller.pics.length
          const ul = this.$refs.picList
          ul.style.width = (liWidth + space) * imgSize - space + 'px'
    ...
    watch: {
      // 初始化时数据还是一个空的, 后面数据准备好后再更新的
      seller (newVal, oldValue) { // seller变化了
        this.updateSeller()
      }
    },
